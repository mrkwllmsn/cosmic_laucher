...
private:
    ...
    void star_field() {
        ...
        if (!stars_initialized) {
            // Initialize slow stars
            for (int i = 0; i < 8; i++) {
                star_field_slow[i][0] = (rand() % 628) / 100.0f; // angle
                star_field_slow[i][1] = (rand() % 300) / 100.0f; // distance
                star_field_slow[i][2] = 0.4f + (rand() % 400) / 1000.0f; // brightness
                star_field_slow[i][3] = 0.3f + (rand() % 200) / 100.0f; // speed
            }
            
            // Initialize medium stars
            for (int i = 0; i < 12; i++) {
                star_field_medium[i][0] = (rand() % 628) / 100.0f;
                star_field_medium[i][1] = (rand() % 400) / 100.0f;
                star_field_medium[i][2] = 0.5f + (rand() % 400) / 1000.0f;
                star_field_medium[i][3] = 0.6f + (rand() % 300) / 100.0f;
            }
            
            // Initialize fast stars
            for (int i = 0; i < 16; i++) {
                star_field_fast[i][0] = (rand() % 628) / 100.0f;
                star_field_fast[i][1] = (rand() % 500) / 100.0f;
        ...
        for (int i = 0; i < 8; i++) {
            ...
            star_field_slow[i][1] += star_field_slow[i][3] * animation_speed * 0.8f;
            
            // Reset star at center when it goes off screen
            if (star_field_slow[i][1] > MAX_DISTANCE) {
                star_field_slow[i][0] = (rand() % 628) / 100.0f;
                star_field_slow[i][1] = (rand() % 200) / 100.0f;
                star_field_slow[i][2] = 0.4f + (rand() % 400) / 1000.0f;
                star_field_slow[i][3] = 0.3f + (rand() % 200) / 100.0f;
            }
        ...
        for (int i = 0; i < 12; i++) {
            // Move star outward from center
            star_field_medium[i][1] += star_field_medium[i][3] * animation_speed * 1.0f;
            
            // Reset star at center when it goes off screen
            if (star_field_medium[i][1] > MAX_DISTANCE) {
                star_field_medium[i][0] = (rand() % 628) / 100.0f;
                star_field_medium[i][1] = (rand() % 200) / 100.0f;
                star_field_medium[i][2] = 0.5f + (rand() % 400) / 1000.0f;
                star_field_medium[i][3] = 0.6f + (rand() % 300) / 100.0f;
            }
        ...
        for (int i = 0; i < 16; i++) {
            ...
            if (star_field_fast[i][1] > MAX_DISTANCE) {
                star_field_fast[i][0] = (rand() % 628) / 100.0f;
                star_field_fast[i][1] = (rand() % 200) / 100.0f;
                star_field_fast[i][2] = 0.6f + (rand() % 400) / 1000.0f;
                star_field_fast[i][3] = 1.0f + (rand() % 400) / 100.0f;
            }
