...
private:
    ...
    std::vector<EyeState> eye_states;
    ...
    void drawCreepyEyes() {
        ...
        // Draw multiple pairs of glowing eyes with individual blinking
        struct EyePair {
            int x, y;
            uint8_t r, g, b;
        };
        ...
        EyePair eyes[] = {
            {8, 10, 255, 0, 0},    // Red eyes
            {22, 8, 0, 255, 0},    // Green eyes
            {15, 20, 255, 255, 0}, // Yellow eyes
            {6, 24, 255, 0, 255},  // Purple eyes
            {26, 22, 0, 255, 255}  // Cyan eyes
        };
        
        for (int i = 0; i < 5; i++) {
            auto& eye_pair = eyes[i];
            
            // Check if this eye should be open (not blinking)
            bool eye_open = !eye_states[i].is_blinking;
            
            if (eye_open) {
                // Left eye
                Pen eye_pen = gfx->create_pen(eye_pair.r, eye_pair.g, eye_pair.b);
                gfx->set_pen(eye_pen);
                gfx->pixel({eye_pair.x, eye_pair.y});
                gfx->pixel({eye_pair.x + 1, eye_pair.y});
                
                // Right eye
                gfx->pixel({eye_pair.x + 3, eye_pair.y});
                gfx->pixel({eye_pair.x + 4, eye_pair.y});
                
                // Glow effect - enhanced for spookier look
                float glow_intensity = 0.4f + 0.2f * sin(background_phase * 3.0f + i * 1.5f);
                Pen glow_pen = gfx->create_pen(
                    (uint8_t)(eye_pair.r * glow_intensity), 
                    (uint8_t)(eye_pair.g * glow_intensity), 
                    (uint8_t)(eye_pair.b * glow_intensity)
                );
                gfx->set_pen(glow_pen);
                
                // Extended glow pattern
                gfx->pixel({eye_pair.x - 1, eye_pair.y});
                gfx->pixel({eye_pair.x + 2, eye_pair.y});
                gfx->pixel({eye_pair.x + 5, eye_pair.y});
                gfx->pixel({eye_pair.x, eye_pair.y - 1});
                gfx->pixel({eye_pair.x + 1, eye_pair.y - 1});
                gfx->pixel({eye_pair.x + 3, eye_pair.y - 1});
                gfx->pixel({eye_pair.x + 4, eye_pair.y - 1});
                gfx->pixel({eye_pair.x, eye_pair.y + 1});
                gfx->pixel({eye_pair.x + 1, eye_pair.y + 1});
                gfx->pixel({eye_pair.x + 3, eye_pair.y + 1});
                gfx->pixel({eye_pair.x + 4, eye_pair.y + 1});
                ...
                Pen outer_glow = gfx->create_pen(
                    (uint8_t)(eye_pair.r * glow_intensity * 0.3f), 
                    (uint8_t)(eye_pair.g * glow_intensity * 0.3f), 
                    (uint8_t)(eye_pair.b * glow_intensity * 0.3f)
                );
                gfx->set_pen(outer_glow);
                gfx->pixel({eye_pair.x - 1, eye_pair.y - 1});
                gfx->pixel({eye_pair.x - 1, eye_pair.y + 1});
                gfx->pixel({eye_pair.x + 2, eye_pair.y - 1});
                gfx->pixel({eye_pair.x + 2, eye_pair.y + 1});
                gfx->pixel({eye_pair.x + 5, eye_pair.y - 1});
                gfx->pixel({eye_pair.x + 5, eye_pair.y + 1});
            }
        }
    }
    ...
    void drawFlyingBats() {
        ...
        // Draw bats flying across screen
        for (size_t i = 0; i < bat_positions.size(); i++) {
            int bat_x = (int)bat_positions[i];
            int bat_y = 8 + i * 6 + (int)(sin(animation_timer * 0.01f + i) * 3);
    ...
    void drawWolfHowling() {
        ...
        // Right mountain (wolf's perch)
        for (int x = 16; x < 32; x++) {
            float peak_height = 22 - (x - 24) * (x - 24) * 0.05f + 
        ...
        // Wolf silhouette on mountain ridge
        int wolf_x = 24;
        int wolf_base_y = 22;
        ...
        if (is_howling) {
            ...
            gfx->pixel({wolf_x + 1, wolf_base_y - 3}); // Extended snout upward
    ...
    void drawSkullCrossbones() {
        ...
        // Glowing eye effect
        float eye_glow = sin(skull_glow_phase * 3.0f) * 0.5f + 0.5f;
        if (eye_glow > 0.6f) {
            uint8_t red_glow = (uint8_t)(eye_glow * 255);
            gfx->set_pen(gfx->create_pen(red_glow, 0, 0));
            gfx->pixel({skull_x - 2, skull_y - 1});
            gfx->pixel({skull_x + 2, skull_y - 1});
            
            // Extended glow
            Pen glow_pen = gfx->create_pen(red_glow/2, 0, 0);
            gfx->set_pen(glow_pen);
            gfx->pixel({skull_x - 3, skull_y - 1});
            gfx->pixel({skull_x + 3, skull_y - 1});
            gfx->pixel({skull_x - 2, skull_y - 2});
            gfx->pixel({skull_x + 2, skull_y - 2});
        }
    }
...
public:
    ...
    void init(PicoGraphics_PenRGB888& graphics, CosmicUnicorn& cosmic_unicorn) override {
        ...
        for (int i = 0; i < 5; i++) {
            EyeState eye_state;
            eye_state.blink_timer = to_ms_since_boot(get_absolute_time());
            eye_state.blink_interval = 2000 + (rand() % 3000); // 2-5 seconds
            eye_state.is_blinking = false;
            eye_states.push_back(eye_state);
        }
    ...
    bool update() override {
        ...
        if (current_time - scene_start_time > current_scene_duration) {
            ...
            if (current_scene == FLYING_BATS) {
                ...
                // Reset eye blinking states
                for (int i = 0; i < 5; i++) {
                    eye_states[i].blink_timer = current_time;
                    eye_states[i].blink_interval = 500 + (rand() % 2000); // Immediate variety
                    eye_states[i].is_blinking = false;
                }
            }
        }
        ...
        switch (current_scene) {
            case CREEPY_EYES:
                // Update individual eye blinking
                for (int i = 0; i < 5; i++) {
                    if (current_time - eye_states[i].blink_timer > eye_states[i].blink_interval) {
                        if (!eye_states[i].is_blinking) {
                            // Start blink
                            eye_states[i].is_blinking = true;
                            eye_states[i].blink_timer = current_time;
                            eye_states[i].blink_interval = 150; // Blink duration
                        } else {
                            // End blink, set new random interval
                            eye_states[i].is_blinking = false;
                            eye_states[i].blink_timer = current_time;
                            eye_states[i].blink_interval = 1500 + (rand() % 4000); // 1.5-5.5 seconds
                        }
                    }
                }
                break;
